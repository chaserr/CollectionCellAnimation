...
class MainCollectionView: UICollectionView,UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout{
    ...
    override init(frame: CGRect, collectionViewLayout layout: UICollectionViewLayout) {
        for image in sectionDatas {
            ...
            let blurImage = image?.blurredImage(withRadius: 20, iterations: 10, tintColor: UIColor.black)
            blurImageArray.add(blurImage!)
        }
        ...
        self.register(UINib(nibName: "MessageCollectionViewCell", bundle:Bundle.main )
, forCellWithReuseIdentifier: identify)
        self.register(UINib(nibName: "SectionView", bundle:Bundle.main), forSupplementaryViewOfKind: UICollectionElementKindSectionHeader, withReuseIdentifier: sectionIdentify)
    }
    ...
    */
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let collectionViewCell:MessageCollectionViewCell? = collectionView.dequeueReusableCell(withReuseIdentifier: identify, for: indexPath as IndexPath) as? MessageCollectionViewCell
        //避免重用的时候出现重用数据的缓存问题
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        if kind == UICollectionElementKindSectionHeader {
             collectionHeader = collectionView .dequeueReusableSupplementaryView(ofKind: UICollectionElementKindSectionHeader, withReuseIdentifier: sectionIdentify, for: indexPath as IndexPath) as? SectionView
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, referenceSizeForHeaderInSection section: Int) -> CGSize {
        if section == 0 {
            return CGSize(width:kScreenWidth, height:300*Screnn_Scale)
        }else{
            return CGSize(width:kScreenWidth, height:220*Screnn_Scale)
        }
    }
